...
import { calculateUsageCost, formatCalculation, ALL_PRICING, UsageEstimate } from './src/estimators/usage-calculator';

console.log('\n☁️💰 Usage-Based Cost Calculator Test\n');
console.log('='.repeat(60));

// Example usage scenarios
const scenarios: { name: string; usage: UsageEstimate }[] = [
  {
...
for (const scenario of scenarios) {
  ...
  const results: { service: string; cost: number; note: string }[] = [];
  
  for (const serviceName of servicesToTest) {
    const calc = calculateUsageCost(serviceName, scenario.usage);
    if (calc) {
      let note = '';
...
const exampleUsage: UsageEstimate = {
...
for (const service of ['CloudFlare Workers', 'Vercel', 'AWS Lambda']) {
  const calc = calculateUsageCost(service, exampleUsage);
  if (calc) {
    console.log(formatCalculation(calc));
  }
}
